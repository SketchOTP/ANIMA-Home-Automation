...
test("suggestions paginate with original evidence and confirmation-only non-executable review", async ({ page }) => {
  ...
  await page.locator(".initiative-suggestion-list > li").first().getByText("Reasoning summary and maturity evidence", { exact: true }).click();
  await expect(page.locator(".initiative-suggestion-list")).toContainText("tentative hypothesis");
  await expect(page.locator(".initiative-suggestion-list")).toContainText("Synced to Obsidian Memory");
  await expect(page.locator(".initiative-suggestion-list")).toContainText("No qualified actor evidence");
  await page.locator(".initiative-suggestion-list > li").first().getByText(/Evidence & provenance/).click();
