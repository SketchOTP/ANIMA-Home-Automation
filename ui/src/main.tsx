...
import { SentryChat } from "./SentryChat";
import { SentryPersonalityProfiles } from "./SentryPersonalityProfiles";
import type { SentryPersonalityProfilesPayload } from "./SentryPersonalityProfiles";
...
type SentryVoiceSettings = { voice_id: string; speech_speed: number; sleep_enabled: boolean; active_instance_id: "living_room" | "office" };
...
function AuthenticationView({ expired = false, notice = "" }: { expired?: boolean; notice?: string }) {
  ...
  const canResume = setup?.state === "ONLINE";
  useEffect(() => { if (canResume) window.location.replace("/auth/login"); }, [canResume]);
  return <main className="shell centered"><div className="brand-mark"><Icon name="Home" /></div><p className="eyebrow">ANIMA · HOME INTELLIGENCE</p><h1>{canResume ? "Returning to ANIMA…" : expired ? "Reconnect your household" : "Your home, connected"}</h1>{notice && <p className="notice error" role="alert">{notice}</p>}<p>{canResume ? "Continuing with your existing household connection." : "Use your existing Home Assistant account. Credentials stay on the server."}</p>{setup?.available && !canResume && <a className="primary-link" href="/auth/login?connect=1"><Icon name="Integrations" /> Connect your Home Assistant</a>}<a className={setup?.available && !canResume ? "" : "primary-link"} href="/auth/login">{canResume ? "Continue now" : "Continue with existing Home Assistant"}</a>{setup && <StatusPill status={setup.state} />}{setupError && <p className="muted" role="status">Setup availability could not be checked.</p>}</main>;
}
...
const sectionActions: Record<string, [string, string][]> = {
  ...
  SENTRY: [["Home status", "What needs attention in my home right now?"], ["Today's agenda", "What is on my household calendar today?"], ["Help me plan", "Help me plan a household task. Ask me what I want to accomplish."]],
  ...
  Connections: [["Check connections", "Review integration health and explain safe recovery options."]],
...
function QuickActions({ section, prepare }: { section: string; prepare: (intent: string) => void }) {
  ...
  return <div className="quick-actions" aria-label={`Assistant quick actions for ${section}`}>{actions.map(([label, intent]) => <button type="button" className="quick-action" key={label} onClick={() => prepare(intent)}><span className="quick-action-icon"><Icon name="Anima" /></span>{label}<Icon name="ArrowRight" /></button>)}</div>;
}
...
function App() {
  const [bootstrap, setBootstrap] = useState<Bootstrap | null>(null); const [home, setHome] = useState<Home | null>(null); const [settings, setSettings] = useState<Settings | null>(null); const [voiceSettings, setVoiceSettings] = useState<SentryVoiceSettings | null>(null); const [personalityProfiles, setPersonalityProfiles] = useState<SentryPersonalityProfilesPayload | null>(null); const [capabilities, setCapabilities] = useState<Capability[]>([]); const [integrations, setIntegrations] = useState<Integration[]>([]); const [devices, setDevices] = useState<ProviderDevice[]>([]); const [spaces, setSpaces] = useState<Space[]>([]); const [alertPolicies, setAlertPolicies] = useState<AlertPolicy[]>([]); const [alertEvents, setAlertEvents] = useState<AlertEvent[]>([]); const [alertNextCursor, setAlertNextCursor] = useState<string | null>(null); const [alertLoading, setAlertLoading] = useState(false); const [notificationRoutes, setNotificationRoutes] = useState<NotificationRoute[]>([]); const [backups, setBackups] = useState<BackupRecord[]>([]); const [scenes, setScenes] = useState<Scene[]>([]); const [automations, setAutomations] = useState<Automation[]>([]); const [preferences, setPreferences] = useState<Preference[]>([]); const [users, setUsers] = useState<User[]>([]); const [tab, setTab] = useState("Home"); const [error, setError] = useState(""); const [outcome, setOutcome] = useState<MutationOutcome | null>(null);
  ...
  const prepare = (intent: string) => { setVoiceSuggestion(intent); setTab("SENTRY"); window.requestAnimationFrame(() => voicePanelRef.current?.focus()); };
  const clearProtectedState = () => {
      authGeneration.current += 1;
      setBootstrap(null); setHome(null); setSettings(null); setVoiceSettings(null); setPersonalityProfiles(null); setConnection(null);
  ...
  const loadSnapshot = async () => {
    ...
    const sections = [
      ...
      section<SentryPersonalityProfilesPayload>("SENTRY personality profiles", "/api/v1/sentry/personality-profiles", setPersonalityProfiles),
  ...
  const navGroups = [
    ["Household", ["Home", "Devices", "Spaces", "Routines", "Scenes", "Automations"]],
    ["SENTRY", ["SENTRY", "Alerts", "Tasks & Calendar", "Activity"]],
    ["Manage", ["Users", "Connections", "Backups", "Preferences", "Settings"]],
  ] as const;
  ...
  const summaries: Record<string, [string, number, string][]> = {
    ...
    Connections: [["Connected services", integrations.length, "Integrations"], ["Enabled", enabled(integrations), "Power"], ["Available capabilities", capabilities.filter((item) => item.state.toLowerCase() === "available").length, "Capabilities"]],
  ...
  const descriptions: Record<string, string> = {
    Routines: "Routines records schedules and expectations you explicitly set for household members: days, local times, an optional room or zone, and a description. SENTRY may use these records with current presence, preferences, memory, and events when reasoning, but a routine is not proof someone is present and does not automatically run an action. Manage people on Users; use Household presence below to associate qualified phone geofence or Wi-Fi evidence.",
    Home: "Your household at a glance.", Devices: "Pair, place, and control your devices.", Spaces: "Use Spaces to map where things are so ANIMA and SENTRY can connect devices and activity to the right part of your home. A room is a physical enclosed area, such as a kitchen, bedroom, or garage. A zone is a more specific or functional area, such as a driveway, front entry, patio, or TV area, that may sit inside or outside a room. Create the room or zone here, then assign devices to it from Devices.",
    Scenes: "Scenes are reusable presets that set several commissioned devices together. Each scene saves an On or Off choice for up to 16 power controls. Choose Apply when you want ANIMA to authorize, run, and verify every saved device change. You can disable a scene without deleting it; if a step fails, ANIMA stops and reports the failed or partial result.",
    Automations: "Automations are saved rules that run without you pressing a button. The current workflow watches one commissioned device for an On or Off observation, then asks one commissioned power control to turn On or Off. Create or edit the rule here, enable it to watch matching observations, or disable it without deleting it. Every triggered action still passes through ANIMA authorization and physical-state verification.",
    Alerts: "Alerts controls which household events deserve attention and how qualifying notifications are routed. Set a device to Always, Time window, Never, or Let SENTRY decide on Devices; use this page for advanced event rules, matched alert history, and the server-owned notification route. A recorded event or provider-accepted delivery does not prove a person received it.",
    SENTRY: "Use SENTRY to describe a household request in natural language. ANIMA queues it for SENTRY's Codex intelligence, supplies only bounded household context and governed tools, and returns the result here. This browser transcript is live-session only and clears on reload; unavailable cognition never silently falls back to another assistant.",
    "Tasks & Calendar": "Tasks are durable reminders or scheduled work that survive restarts; Calendar stores household events with start and end times. Create, pause, resume, or cancel tasks here, and create, edit, or cancel calendar entries. Calendar updates use version checks so a stale edit is rejected instead of overwriting newer changes.",
    Activity: "Activity is a read-only view of recent household observations and recorded outcomes from ANIMA. Use it to see what the system recently noticed or completed. It does not change devices, replace the complete audit journal, or prove that a notification was delivered or read.",
    Connections: "Connections shows ANIMA's registered services, current health, available capabilities, and supported setup flows. Configure ZHA and Ring here, check Tapo and Wansview notification intake, reconnect Home Assistant, and enable or disable only integrations marked manageable. Credentials remain server-owned; capability entries report availability and do not grant permission.",
    Backups: "Backups creates server-owned snapshots of ANIMA's PostgreSQL household records. Create a snapshot, validate its integrity, or restore a valid one with explicit confirmation. Restore replaces ANIMA records but never assumes restored device state is current—Home Assistant must provide fresh observations afterward.",
    Preferences: "Preferences stores household-wide and person-specific guidance that SENTRY may consider with current events, routines, presence, and memory. Preferences are context—not commands, automations, permissions, or verified Truth. This page also manages notification-learning policy and curated long-term MEMORY notes.",
    Settings: "Settings controls how the ANIMA interface looks, which Home widgets appear, and the active SENTRY projection's voice, speech speed, sleep mode, and conversational personality. Personality profiles are free-form presentation guidance only; they cannot change household Truth, alerts, permissions, policy, or verified device results.",
    Users: "Users manages the people ANIMA knows about: household role, Limited or Unrestricted SENTRY access, Wi-Fi presence hints, and private local face profiles. Wi-Fi and face recognition contribute context but do not authenticate a person by themselves. Saved profiles and settings persist across restarts.",
  };
  return <div className="app-shell">
    ...
    <main className="content" id="main-content" tabIndex={-1}>
      ...
      <section className="section-intro" aria-label={`${tab} overview`}><div className="section-heading"><p className="section-description">{descriptions[tab]}</p><span className="assistant-note">Shortcuts prepare a request for SENTRY; review it before sending.</span></div><QuickActions section={tab} prepare={prepare} />{summaries[tab] && <SummaryCards items={summaries[tab]} />}</section>
      {tab === "Home" && <HomeView home={home} settings={settings} mutate={mutate} openSentry={() => setTab("SENTRY")} />}
      {tab === "Devices" && <DevicesView devices={devices} rooms={home.rooms} mutate={mutate} onAuthFailure={expireSession} />}
      {tab === "Spaces" && <SpacesView spaces={spaces} mutate={mutate} />}
      {tab === "Routines" && <><FamilyRoutines mutate={mutate} onAuthFailure={expireSession} openUsers={() => setTab("Users")} /><HouseholdPresencePanel mutate={mutate} onAuthFailure={expireSession} /></>}
...
function HomeView({ home, settings, mutate, openSentry }: { home: Home; settings: Settings; mutate: (path: string, payload?: Record<string, unknown>) => Promise<MutationOutcome | null>; openSentry: () => void }) {
  ...
  const widgets: Record<string, ReactNode> = {
    ...
    conversation: <Card title="Ask SENTRY"><p className="muted">Describe a household outcome in everyday language.</p><button type="button" onClick={openSentry}>Open SENTRY</button></Card>,
...
function DevicesView({ devices, rooms, mutate, onAuthFailure }: { devices: ProviderDevice[]; rooms: Room[]; mutate: (path: string, payload?: Record<string, unknown>) => Promise<MutationOutcome | null>; onAuthFailure: () => void }) {
  ...
  return <div className="dashboard">
    ...
    <Card title="Household devices" className="device-catalog"><div className="filter-bar"><label>Search devices<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, model, or room" /></label><label>Device status<select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">All devices</option><option value="commissioned">Ready in ANIMA</option><option value="new">Needs a room</option><option value="attention">State needs attention</option></select></label><label>Filter by room<select value={roomFilter} onChange={(event) => setRoomFilter(event.target.value)}><option value="all">All rooms</option>{rooms.map((room) => <option key={room.place_id} value={room.place_id}>{room.name}</option>)}</select></label></div><p className="muted" role="status">{filtered.length} of {discovered.length} devices from Home Assistant and supported event connections</p>{filtered.length ? <ul className="clean-list list-spaced device-grid">{filtered.map((item) => { const mapped = item.metadata.mapping_status === "MAPPED"; const resourceId = String(item.metadata.canonical_target_id ?? ""); const power = item.capabilities?.find((capability) => capability.type === "power.set" && capability.writable); const label = deviceDisplayName(item, rooms); const observed = item.observed_at ? new Date(item.observed_at).toLocaleString() : "No current observation"; const eventOnly = item.metadata.source_kind === "notification_resource"; return <li className="device-row" key={item.device_handle}><span><span className="device-icon"><Icon name={power ? "Power" : eventOnly ? "Notifications" : "Devices"} /></span><strong>{label}</strong><small className="muted">{deviceRoom(item)?.name ?? (mapped ? "No room assigned" : "Awaiting commissioning")}</small><small className="muted">{item.metadata.manufacturer ?? ""}{item.metadata.model ? ` · ${item.metadata.model}` : ""} · {eventOnly ? "Event connection" : mapped ? "Ready in ANIMA" : "Needs a room"}</small>{mapped && !eventOnly && <small className="device-state">Current state: {["CURRENT", "CURRENT/KNOWN"].includes(item.truth_status ?? "") ? item.state ?? "UNKNOWN" : "UNKNOWN"}</small>}{eventOnly && <small className="device-state">Reports events only · no direct control</small>}{mapped && !eventOnly && <small className="muted">Truth: <StatusPill status={item.truth_status ?? "UNKNOWN"} /></small>}{mapped && <LastReported reading={item} />}{mapped && !eventOnly && <small className="muted">Observed: {observed}</small>}{mapped && item.capabilities && item.capabilities.length > 0 && <CapabilityDetails capabilities={item.capabilities} label={label} />}</span><span className="button-row">{mapped && power && resourceId && <PowerControls id={resourceId} label={label} state={power.truth_status === "CURRENT" ? power.state : "UNKNOWN"} mutate={mutate} />}{mapped && !eventOnly && <button type="button" disabled={!canonicalPlacement(item) || syncingId !== null} aria-describedby={!canonicalPlacement(item) ? `sync-mapping-${item.device_handle}` : undefined} onClick={() => void syncCapabilities(item)}><Icon name="Refresh" />{syncingId === item.device_handle ? "Syncing capabilities…" : "Sync capabilities"}</button>}{mapped && !eventOnly && !canonicalPlacement(item) && <small className="muted" id={`sync-mapping-${item.device_handle}`}>Cannot sync: canonical device name or room mapping is missing or ambiguous. Refresh household data first.</small>}{mapped && !eventOnly ? <button onClick={() => beginEdit(item)}>Manage</button> : !mapped ? <button onClick={() => beginCommission(item)}>Add to ANIMA</button> : null}</span></li>; })}</ul> : <p className="empty-state">{discovered.length ? "No devices match these filters." : "No household devices found. Pair or connect a device, then refresh."}</p>}
    ...
    <DeviceNotificationsPanel devices={discovered} mutate={mutate} onAuthFailure={onAuthFailure} />
  </div>;
}
...
function AlertPoliciesView({ policies, devices, rooms, mutate, events, nextCursor, loading, loadOlder }: { policies: AlertPolicy[]; devices: ProviderDevice[]; rooms: Room[]; mutate: (path: string, payload?: Record<string, unknown>) => Promise<MutationOutcome | null>; events: AlertEvent[]; nextCursor: string | null; loading: boolean; loadOlder: () => Promise<void> }) {
  ...
  return <div className="dashboard"><Card title={editing ? "Edit advanced event rule" : "Advanced event rule"}><p className="muted">Most device notification choices belong on the Devices page. Use this only when a specific sensor event needs its own time window, priority, or delivery path.</p>{resources.length ? <form className="stack" onSubmit={(event) => void save(event)}><label>Devices or sensors<select multiple required value={draft.resource_ids} onChange={(event) => setDraft({ ...draft, resource_ids: Array.from(event.target.selectedOptions, (option) => option.value) })}>{resources.map((resource) => <option key={resource.id} value={resource.id}>{resource.label}</option>)}</select></label><label>Event name<input required maxLength={120} value={draft.event_type} onChange={(event) => setDraft({ ...draft, event_type: event.target.value })} /><small className="muted">Use the normalized ANIMA event name shown in Activity.</small></label><div className="form-grid"><label>From<input required type="time" value={draft.start_local} onChange={(event) => setDraft({ ...draft, start_local: event.target.value })} /></label><label>Until<input required type="time" value={draft.end_local} onChange={(event) => setDraft({ ...draft, end_local: event.target.value })} /></label><label>Priority<input required type="number" min="0" max="100" value={draft.priority} onChange={(event) => setDraft({ ...draft, priority: Number(event.target.value) })} /></label><label>Household timezone<input required maxLength={64} value={draft.timezone} onChange={(event) => setDraft({ ...draft, timezone: event.target.value })} /></label></div><label>What should happen<select value={draft.delivery_mode} onChange={(event) => setDraft({ ...draft, delivery_mode: event.target.value as AlertPolicy["delivery_mode"] })}><option value="SENTRY_COGNITION">Ask SENTRY to evaluate it</option><option value="NOTIFICATION">Use the notification route</option></select></label><label className="checkbox"><input type="checkbox" checked={draft.guaranteed_attention} onChange={(event) => setDraft({ ...draft, guaranteed_attention: event.target.checked })} /> Treat every matching event as high priority</label><label className="checkbox"><input type="checkbox" checked={draft.enabled} onChange={(event) => setDraft({ ...draft, enabled: event.target.checked })} /> Rule enabled</label><div className="button-row"><button type="submit">{editing ? "Save changes" : "Create rule"}</button>{editing && <button type="button" onClick={reset}>Cancel</button>}</div></form> : <p className="muted">No commissioned devices or sensors are available yet. Add one on the Devices page first.</p>}</Card><Card title="Configured advanced rules"><p className="muted">Disabling a rule preserves its history.</p>{policies.length ? <ul className="clean-list list-spaced">{policies.map((policy) => <li className="policy-row" key={policy.policy_id}><span><strong>{policy.event_type}</strong><small className="muted">{policy.start_local.slice(0, 5)}–{policy.end_local.slice(0, 5)} · priority {policy.priority} · v{policy.version}</small></span><span className="button-row"><StatusPill status={policy.enabled ? "ACTIVE" : "DISABLED"} /><button onClick={() => edit(policy)}>Edit</button><button aria-pressed={policy.enabled} onClick={() => toggle(policy)}>{policy.enabled ? "Disable" : "Enable"}</button></span></li>)}</ul> : <p className="muted">No advanced event rules configured.</p>}</Card><Card title="Alert inbox"><p className="muted">Recorded matched events. Delivery status does not confirm human receipt.</p>{events.length ? <ul className="clean-list list-spaced">{events.map((event) => <li className="alert-row" key={event.alert_id}><span><strong>{event.resource_name}</strong><small className="muted">{event.event_type} · {new Date(event.occurred_at).toLocaleString()} · priority {event.priority}</small><small className="muted">{event.delivery_mode.replaceAll("_", " ")}</small></span><span className="button-row"><StatusPill status={event.delivery_status} /></span></li>)}</ul> : <p className="muted">No matched alert events recorded yet.</p>}{nextCursor && <div className="button-row"><button type="button" onClick={() => void loadOlder()} disabled={loading}>{loading ? "Loading older alerts…" : "Load older alerts"}</button></div>}</Card></div>;
}
...
function SettingsPanel({ value, voice, personalities, csrf, onSaved, onVoiceSaved, onPersonalitiesChanged, setError, setOutcome }: { value: Settings; voice: SentryVoiceSettings | null; personalities: SentryPersonalityProfilesPayload | null; csrf: string; onSaved: (value: Settings) => void; onVoiceSaved: (value: SentryVoiceSettings) => void; onPersonalitiesChanged: (value: SentryPersonalityProfilesPayload) => void; setError: (value: string) => void; setOutcome: (value: MutationOutcome) => void }) {
  ...
  const save = async () => { try { const result = await api<{ settings: Settings }>("/api/v1/settings", { method: "PUT", body: JSON.stringify({ payload: draft }), headers: { "Content-Type": "application/json", "X-Anima-CSRF": csrf, Origin: window.location.origin } }); onSaved(result.settings); setOutcome({ status: "SUCCEEDED", operation: "settings.update", detail: "Interface settings saved" }); setError(""); } catch (err) { setError(err instanceof Error ? err.message : "Settings could not be saved"); } }; const widgets = ["status", "presence", "weather", "agenda", "tasks", "controls", "conversation", "activity", "household", "reports", "health"];
  const [voiceDraft, setVoiceDraft] = useState(voice || { voice_id: "bm_george", speech_speed: 0.9, sleep_enabled: false, active_instance_id: "living_room" as const }); useEffect(() => { if (voice) setVoiceDraft(voice); }, [voice]);
  ...
  return <><Card title="Household interface"><p className="muted">Choose the appearance, layout profile, text size, motion level, and Home widgets used by this ANIMA screen. These controls affect presentation only; they do not change alerts, household preferences, permissions, or device state.</p><div className="settings-form"><label>Appearance<select value={draft.appearance} onChange={(event) => update("appearance", event.target.value as Settings["appearance"])}><option value="system">System</option><option value="light">Light</option><option value="night">Night</option></select></label><label>Accent<select value={draft.accent} onChange={(event) => update("accent", event.target.value as Settings["accent"])}><option value="purple">Neon purple</option><option value="ember">Ember</option><option value="sage">Sage</option><option value="sky">Sky</option></select></label><label>Density<select value={draft.density} onChange={(event) => update("density", event.target.value as Settings["density"])}><option value="comfortable">Comfortable</option><option value="compact">Compact</option></select></label><label>Text scale<select value={draft.text_scale} onChange={(event) => update("text_scale", event.target.value as Settings["text_scale"])}><option value="small">Small</option><option value="normal">Normal</option><option value="large">Large</option></select></label><label>Layout profile<select value={draft.display_mode} onChange={(event) => update("display_mode", event.target.value as Settings["display_mode"])}><option value="wall">Wall</option><option value="tablet">Tablet</option><option value="phone">Phone</option><option value="desktop">Desktop</option></select></label><label className="checkbox"><input type="checkbox" checked={draft.reduced_motion} onChange={(event) => update("reduced_motion", event.target.checked)} /> Reduce motion</label></div><fieldset className="widget-settings"><legend>Home widgets</legend>{widgets.map((id) => { const index = draft.widget_order.indexOf(id); return <label className="widget-setting" key={id}><input type="checkbox" checked={draft.visible_widgets.includes(id)} onChange={() => toggleWidget(id)} /><span>{id}</span><button type="button" aria-label={`Move ${id} up`} disabled={index <= 0} onClick={() => moveWidget(id, -1)}>↑</button><button type="button" aria-label={`Move ${id} down`} disabled={index < 0 || index >= draft.widget_order.length - 1} onClick={() => moveWidget(id, 1)}>↓</button></label>; })}</fieldset><button onClick={() => void save()}>Save interface settings</button></Card><Card title="SENTRY voice"><p className="muted">ANIMA owns which SENTRY face is active, its voice, speaking pace, and wake availability. Switching keeps one intelligence on this PC and moves only its camera, microphone, speaker, and visible face.</p><div className="settings-form"><label>Active SENTRY location<select value={voiceDraft.active_instance_id} onChange={event => setVoiceDraft({ ...voiceDraft, active_instance_id: event.target.value as SentryVoiceSettings["active_instance_id"] })}><option value="living_room">Living room TV · Raspberry Pi 5</option><option value="office">Office · this PC</option></select></label><label>Voice<select value={voiceDraft.voice_id} onChange={event => setVoiceDraft({ ...voiceDraft, voice_id: event.target.value })}><option value="bm_george">George</option><option value="bm_lewis">Lewis</option><option value="am_adam">Adam</option><option value="am_michael">Michael</option><option value="af_bella">Bella</option><option value="af_sarah">Sarah</option><option value="bf_emma">Emma</option><option value="bf_isabella">Isabella</option></select></label><label>Speech speed<input type="range" min="0.75" max="1.30" step="0.05" value={voiceDraft.speech_speed} onChange={event => setVoiceDraft({ ...voiceDraft, speech_speed: Number(event.target.value) })} /><output>{voiceDraft.speech_speed.toFixed(2)}×</output></label><label className="checkbox"><input type="checkbox" checked={voiceDraft.sleep_enabled} onChange={event => setVoiceDraft({ ...voiceDraft, sleep_enabled: event.target.checked })} /> Sleep mode</label></div><p className="muted">Off means standby: the selected location listens for the wake word. Changing location safely restarts only SENTRY's voice edge; ANIMA, household events, and the persistent SENTRY intelligence remain running.</p><button onClick={() => void saveVoice()}>Save SENTRY location, voice, and mode</button></Card><SentryPersonalityProfiles value={personalities} csrfToken={csrf} onChanged={onPersonalitiesChanged} onOutcome={setOutcome} onError={setError} /></>;
}
function ScenesView({ scenes, controls, mutate }: { scenes: Scene[]; controls: Home["controls"]; mutate: (path: string, payload?: Record<string, unknown>) => Promise<MutationOutcome | null> }) {
  ...
  const toggle = (scene: Scene) => void mutate("/api/v1/scenes", { scene_id: scene.scene_id, expected_version: scene.version, name: scene.name, steps: scene.steps, enabled: !scene.enabled });
  return <div className="dashboard"><Card title={editing ? "Edit scene" : "Create a scene"}><p className="muted">Choose up to 16 commissioned power devices and save the On or Off state each should use. Saving the scene does not change any devices.</p>{controls.length ? <form className="stack" onSubmit={(event) => void save(event)}><label>Scene name<input required maxLength={80} value={name} onChange={(event) => setName(event.target.value)} /></label><div className="form-grid"><label>Device<select value={resourceId} onChange={(event) => setResourceId(event.target.value)}>{controls.map((control) => <option key={control.control_id} value={control.control_id}>{control.label}</option>)}</select></label><label>State<select value={desiredOn ? "on" : "off"} onChange={(event) => setDesiredOn(event.target.value === "on")}><option value="on">On</option><option value="off">Off</option></select></label></div><button type="button" onClick={addStep} disabled={!resourceId || steps.some((step) => step.resource_id === resourceId)}>Add device state</button>{steps.length ? <ol className="clean-list list-spaced">{steps.map((step, index) => <li key={step.resource_id}><span>{controls.find((control) => control.control_id === step.resource_id)?.label ?? step.resource_id}<small className="muted">Set {step.desired_on ? "On" : "Off"}</small></span><button type="button" onClick={() => setSteps(steps.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></li>)}</ol> : <p className="muted">Add at least one commissioned power device.</p>}<div className="button-row"><button type="submit" disabled={!name.trim() || !steps.length}>{editing ? "Save scene" : "Create scene"}</button>{editing && <button type="button" onClick={reset}>Cancel</button>}</div></form> : <p className="muted">No commissioned power devices are available yet. Add and commission a power device on Devices first.</p>}</Card><Card title="Saved scenes"><p className="muted">Apply runs each saved device change through ANIMA authorization and physical-state verification. If one change fails, ANIMA stops and reports the failed or partial result. Disabling a scene prevents it from being applied but does not change any devices.</p>{scenes.length ? <ul className="clean-list list-spaced">{scenes.map((scene) => <li className="integration-row" key={scene.scene_id}><span><strong>{scene.name}</strong><small className="muted">{scene.steps.length} device state(s) · v{scene.version}</small></span><span className="button-row"><StatusPill status={scene.enabled ? "ACTIVE" : "DISABLED"} /><button disabled={!scene.enabled} onClick={() => void mutate(`/api/v1/scenes/${scene.scene_id}/apply`)}>Apply scene</button><button aria-pressed={scene.enabled} onClick={() => toggle(scene)}>{scene.enabled ? "Disable" : "Enable"}</button><button onClick={() => beginEdit(scene)}>Edit</button></span></li>)}</ul> : <p className="muted">No scenes configured.</p>}</Card></div>;
}
function AutomationsView({ automations, devices, controls, mutate }: { automations: Automation[]; devices: ProviderDevice[]; controls: Home["controls"]; mutate: (path: string, payload?: Record<string, unknown>) => Promise<MutationOutcome | null> }) {
  const mapped = devices.filter((item) => item.metadata.mapping_status === "MAPPED" && item.metadata.canonical_target_id && item.metadata.source_kind !== "notification_resource");
