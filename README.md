...
- Owner-facing Alert inbox from the Alerts view: review matched SenseGuard events from the authoritative Event Journal with canonical resource names, source-observation times, configured delivery mode, priority, and bounded notification disposition. Delivery status is not a human-receipt claim.
...
- Server-owned PostgreSQL backup snapshots, integrity validation, and explicitly confirmed restore are available from the authenticated Backups view. Archive paths, credentials, household scope, and restore authority remain inside Core.
...
- The Integrations view can start and complete the bounded ZHA integration setup flow through Core using server-held Home Assistant credentials; unsupported integration administration remains explicit rather than becoming a raw HA console.
...
Direct SENTRY voice behavior remains owned by the SENTRY project. Retailer checkout/cart automation, browser/private endpoint access, Mem0, local embeddings, CEL, NATS, and policy-editing runtime APIs are not included. Backup restore is an explicitly confirmed Core-owned recovery operation; restored physical state must be re-observed before it is treated as current. The Phase 6/9 integration is limited to an isolated HA test instance and low-risk virtual entities; Phase 11 uses Open-Meteo, private SearXNG, OpenStreetMap Overpass, TheMealDB, UPCitemdb, the first-party local calendar, and ntfy, and no human-delivery claim is made.
