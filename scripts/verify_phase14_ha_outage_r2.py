"""Prove real isolated-HA outage recovery does not redispatch an action."""
...
def main() -> int:
    ...
    try:
        ...
        action = ActionRequest.create(
            ...
            idempotency_key=f"phase14-ha-outage-{uuid4()}",
        ...
        # Establish a known pre-outage state through the real HA adapter.
        settled = adapter.set_power(resource_id, False, capability_id)
        assert settled.observed_state == "off", settled
        fixture.stop()
        wait_for(lambda: adapter.status.health.value == "OFFLINE", timeout=15)
        outage = coordinator.execute(action)
        assert outage.record.status == ActionStatus.UNKNOWN_RESULT
        assert gateway.calls == 0

        fixture.restart()
        assert adapter.reconnect(lambda: connection_factory(token)) is True
        assert adapter.status.health.value == "ONLINE"
        replay = coordinator.execute(action)
        assert replay.duplicate is True
        assert replay.record.status == ActionStatus.UNKNOWN_RESULT
        assert gateway.calls == 0
        current = adapter.read_state(resource_id, capability_id)
        assert current["state"] == "off"
        print(
            json.dumps(
                {
                    "scenario_id": "HA_OUTAGE_NO_REDISPATCH",
                    "status": "PASS",
                    "evidence_level": "ISOLATED_HA_POSTGRES_OPA",
                    "outage_terminal": outage.record.status.value,
                    "reconnect_health": adapter.status.health.value,
                    "replay_duplicate": replay.duplicate,
                    "provider_dispatches": gateway.calls,
                    "observed_state_after_reconnect": current["state"],
                    "phase15": False,
                },
                sort_keys=True,
            )
